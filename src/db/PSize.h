...
public:
  ...
  /* ISLARGEENOUGHFOR - a.ISLARGEENOUGHFOR(b) is true if an image of size A would not have to be scaled up to fit in B.
     In other words, at least one dimension of A is no smaller than the
  ...
  */
  ...
  // SCALEDDOWNTOFITIN - If A already fits in B, a.SCALEDDOWNTOFITIN(b) simply returns B. Otherwise, it returns a scaled down version that does fit snugly.

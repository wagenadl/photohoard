...
system("rm nikonlenses.db");
open SQL, "|sqlite3 nikonlenses.db" or die "Cannot create db\n";
...
# nikon lens ID numbers (ref 8/11) - from ExifTool's Nikon.pm
my %nikonLensIDs = (
    # Notes => q{
...
for my $k (keys %nikonLensIDs) {
  my $desc = $nikonLensIDs{$k};
  $k =~ s/ //g;
  $k =~ s/\..*$//;
  my $id = hex("0x$k");
  print SQL "insert into lenses values ($id, \"$desc\");\n";
}
